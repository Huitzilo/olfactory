...
n, bins, patches = pl.hist(corr[:, 2], bins=20, histtype='stepfilled', normed=1)
#plot gauss curve
...
pl.yticks(np.arange(nmax / n_ticks, nmax * (1 + 2 / n_ticks), nmax / n_ticks),
          np.round(np.arange(scale / n_ticks, scale * (1 + 2 / n_ticks), scale / n_ticks), 2))
...
#    if a > b:
#        print "\t \\textbf{%s} & %s & %1.2f \\\\ \\hline" % (hallem.odorant_list[i[0]], hallem.odorant_list[i[1]], i[2])
#    else:
#        print "\t %s & \\textbf{%s} & %1.2f \\\\ \\hline" % (hallem.odorant_list[i[0]], hallem.odorant_list[i[1]], i[2])
#
#
...
#    ax1.bar(np.arange(0.2, len(o1_spec) + .2, 1), o1_spec[sorting], width=0.3, color='b', alpha=.6)
#    ax1.bar(np.arange(.5, len(o2_spec) + .5, 1), o2_spec[sorting], width=0.3, color='r', alpha=.6)
...
#
#    pl.savefig("../figures/hallem/correlation/odorants/" + str(c) + "_" + o1_name + "_" + o2_name + ".png")
#
#
#
#    #
#    ## what happens, if we perform be after we remove some redundant data, e.g. remove those odorants which
#    ## have a high correlation with another odorant
#    #
#    #max_corr = strong_corr[np.argsort(strong_corr[:, 2])[-2]] # select the pair with the strongest pos. correlation
#    #print max_corr
#    #
