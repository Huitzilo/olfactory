...
# encoding: utf-8
'''
Compute the euclidean distance of the reference set (deBruyne) on Hallem and DoOR.
'''
import numpy as np
import toolbox
...
toolbox.plot_fingerprints(title, odorants, data, or_list, path)
...
print "Euclidean distance DoOR:", np.min(distance.pdist(door_data, 'euclidean'))
title = "Diagnostic set from DeBruyne 2001 on dorsal DoOR"
path = "figures/debruyne_door.png"
toolbox.plot_fingerprints(title, door_odorants, door_data, door_ors, path)