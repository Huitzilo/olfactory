'''
The goal is to quantify how large the euclidean distance has to be, to distinguish two receptors.
...
for e_ind, e in enumerate(euc):
  for d_ind, d in enumerate(num_dim):
    ...
    # the distance is defined as euclidean distance divided by the square root of the number of features
    mu = e / np.sqrt(d)
    ...
    # compute the squared euclidean distance for each sample to both of the origin points for each receptor type
    d1 = np.vstack((np.sum((o1 - spec1) ** 2, 1), np.sum((o1 - spec2) ** 2, 1)))
    d2 = np.vstack((np.sum((o2 - spec1) ** 2, 1), np.sum((o2 - spec2) ** 2, 1)))

    #count where the distance of the sample is smaller to the second origin, than to the first origin
    #vice versa
    miscl[e_ind, d_ind] = np.sum(np.argmin(d1, 0) == 1) + np.sum(np.argmin(d2, 0) == 0)
...
for e_ind, e in enumerate(euc):
  for d_ind, d in enumerate(num_dim):
    ...
    misc2[e_ind, d_ind] = np.sum(np.argmin(d1, 0) == 1) + np.sum(np.argmin(d2, 0) == 0)
...
'''
Scatterplot a 2D example of two receptors with different centers and different distances to each other
'''
#num_samples = 250
...
#
#        ax.plot(o1[np.where(mask1 == 0), 0], o1[np.where(mask1 == 0), 1], 'ko', alpha=.25)
#        ax.plot(o2[np.where(mask2 == 0), 0], o2[np.where(mask2 == 0), 1], 'bo', alpha=.25)
#        ax.plot(o1[np.where(mask1 == 1), 0], o1[np.where(mask1 == 1), 1], 'ro')
#        ax.plot(o2[np.where(mask2 == 1), 0], o2[np.where(mask2 == 1), 1], 'mo')
